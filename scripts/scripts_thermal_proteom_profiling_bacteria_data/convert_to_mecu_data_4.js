...
const start = function(filePath) {
    ...
    // result header data
    const resultHeader = "Accession	Description	GeneName	Peptides	PSMs	AAs	MW.kDA	pI	QuantifyingPSMs	T37	T40	T47	T53	T59	T63	T69	T75	T81	T87\n";
    ...
    // rel_fc_126_Lysate_1
    const tempStrConstructor = (z1, z2) => `rel_fc_${z1}_Lysate_${z2}`;
    // rel_fc_126_Lysate_Mg_1
    const tempStrConstructor2 = (z1, z2) => `rel_fc_${z1}_Lysate_Mg_${z2}`;
    const fcs = ["126", "127L", "127H", "128L", "128H", "129L", "129L", "130L", "130H", "131L"];
    let experimentHeaderArray = createExperimentHeaderArrays(array, tempStrConstructor, [1,2,3], fcs);
    experimentHeaderArray = experimentHeaderArray
        .concat(createExperimentHeaderArrays(array, tempStrConstructor2, [1,2,3], fcs));
    ...
    // write to files
    experimentResultStrings.forEach((str, i) => {
        fs.writeFileSync(`scripts/scripts_thermal_proteom_profiling_bacteria_data/experiment_4_`+i+'.txt', str, {flag: `w`}, function (err) {
            if (err) {
